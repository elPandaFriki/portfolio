import { createTheme, responsiveFontSizes, ThemeProvider as Provider } from "@mui/material";
...
class Theme extends Component<ThemeProps, {}> {
  ...
  addResponsiveness = (theme: any): any => {
    /**
  ...
  getTheme = () => {
    ...
    const breakpointsKeys: any = Object.keys(breakpoints);
    const theme = responsiveFontSizes(
      this.addResponsiveness(
        createTheme({
          palette,
          typography: this.getTypography(),
          spacing: (factor: any): string => {
    ...
    if (isMain) document.body.setAttribute("theme", mode);
    return theme;
  };

  render() {
    const { children } = this.props;
    return <Provider theme={this.getTheme()}>{children}</Provider>;
  }
}
